...
interface MinigamesFormProps {
  onUpdate: (summary: { service: string; details: string[] }) => void;
}

export function MinigamesForm({ onUpdate }: MinigamesFormProps) {
  ...
  useEffect(() => {
    const selectedMinigameNames = OSRS_MINIGAMES
      .filter(m => selectedMinigames.includes(m.id))
      .map(m => m.label);

    const details = [
      `Minigames: ${selectedMinigames.length} selected`,
      ...selectedMinigameNames.slice(0, 2),
      ...(selectedMinigameNames.length > 2 ? [`+ ${selectedMinigameNames.length - 2} more`] : []),
    ...
    onUpdate({
      service: 'Minigames & Rewards',
      details,
    });
  }, [selectedMinigames, accountType, additionalInfo, files, onUpdate]);
  ...
  const filteredMinigames = OSRS_MINIGAMES.filter(m =>
      ...
      <div className="space-y-8">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            <label className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.3em]">Minigame Selection</label>
            <p className="text-[11px] text-zinc-500 font-medium">Select the activities or rewards you need</p>
          </div>
          <div className="flex items-center gap-3">
            {selectedMinigames.length > 0 && (
              <button
                onClick={() => setSelectedMinigames([])}
            ...
            <div className="px-3 py-1 bg-amber-500/10 border border-amber-500/20 rounded-full">
              <span className="text-[10px] font-black text-amber-500 uppercase tracking-widest">
                {selectedMinigames.length} selected
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="relative group">
          ...
          <input
            type="text"
            placeholder="Search for a minigame or reward..."
            value={searchQuery}
        ...
        <div className="max-h-[400px] overflow-y-auto pr-2 custom-scrollbar grid grid-cols-1 md:grid-cols-2 gap-3">
          {filteredMinigames.length > 0 ? (
            filteredMinigames.map((minigame) => (
              ...
              >
                ...
                <span className={cn(
                  ...
                  selectedMinigames.includes(minigame.id) ? "text-zinc-950" : "text-zinc-100"
                )}>{minigame.label}</span>
                <div className={cn(
                  ...
                  {selectedMinigames.includes(minigame.id) && <Check className="w-4 h-4 text-amber-500 stroke-[3]" />}
                </div>
              </button>
            ))
          ) : (
            <div className="col-span-full py-20 text-center space-y-4">
              ...
              <p className="text-sm text-zinc-500 font-medium">No minigames found matching &quot;{searchQuery}&quot;</p>
            </div>
          )}
        </div>
      </div>
