...
import { Info, Upload, FileText, Check } from 'lucide-react';
import { AccountTypeSelector } from './AccountTypeSelector';

interface IronmanGatheringFormProps {
  onUpdate: (summary: { service: string; details: string[] }) => void;
}

export function IronmanGatheringForm({ onUpdate }: IronmanGatheringFormProps) {
  ...
  useEffect(() => {
    ...
    onUpdate({
      service: 'Ironman Gathering',
      details,
    });
  }, [request, accountType, additionalInfo, files, onUpdate]);
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              <label className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.3em]">What needs to be collected?</label>
              <p className="text-[11px] text-zinc-500 font-medium">Specify resources, quantities, and preferred methods</p>
            </div>
            <div className={cn(
              ...
              {request.length >= 10 ? <Check className="w-3 h-3" /> : <Info className="w-3 h-3" />}
              <span className="text-[9px] font-black uppercase tracking-widest">
                {request.length >= 10 ? 'Validated' : 'Required'}
              </span>
            </div>
          </div>
          <div className="relative">
            <textarea
              ...
              placeholder="e.g. 5,000 Giant Seaweed, 10,000 Sandstone, 1,000 Dragon Bones..."
