...
interface BloodTorvaFormProps {
  onUpdate: (summary: { service: string; details: string[] }) => void;
}

export function BloodTorvaForm({ onUpdate }: BloodTorvaFormProps) {
  ...
  useEffect(() => {
    const selectedBossLabels = OSRS_BLOOD_TORVA_BOSSES
      .filter(b => selectedBosses.includes(b.id))
      .map(b => b.label);

    const details = [
      `Bosses: ${selectedBossLabels.length > 0 ? selectedBossLabels.slice(0, 2).join(', ') + (selectedBossLabels.length > 2 ? ` (+${selectedBossLabels.length - 2})` : '') : 'None'}`,
  ...
  }, [selectedBosses, accountType, additionalInfo, files, onUpdate]);
    ...
    <div className="space-y-12">
      <div className="space-y-8">
        <div className="space-y-4">
          <label className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.3em]">Select Bosses</label>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {OSRS_BLOOD_TORVA_BOSSES.map((boss) => (
              <button
                key={boss.id}
              ...
              >
                ...
                <span className={cn(
                  ...
                  selectedBosses.includes(boss.id) ? "text-zinc-950" : "text-zinc-100"
                )}>{boss.label}</span>
                <div className={cn(
                  ...
                  {selectedBosses.includes(boss.id) && <Check className="w-4 h-4 text-amber-500 stroke-[3]" />}
                </div>
              </button>
            ))}
          </div>
