...
import { OSRS_RAIDS } from '@/data/boosting/osrs-raids';
import { AccountTypeSelector } from './AccountTypeSelector';

interface RaidsFormProps {
  onUpdate: (summary: { service: string; details: string[] }) => void;
}

export function RaidsForm({ onUpdate }: RaidsFormProps) {
  ...
  useEffect(() => {
    const selectedRaidLabels = OSRS_RAIDS
      .filter(r => selectedRaids.includes(r.id))
      .map(r => r.label);

    const details = [
      `Raids: ${selectedRaidLabels.length > 0 ? selectedRaidLabels.slice(0, 2).join(', ') + (selectedRaidLabels.length > 2 ? ` (+${selectedRaidLabels.length - 2})` : '') : 'None'}`,
  ...
  }, [selectedRaids, accountType, additionalInfo, files, onUpdate]);
        ...
        <div className="space-y-6">
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center justify-between">
              <label className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.3em]">Select Raids</label>
              <div className="flex items-center gap-3">
                ...
                <div className="px-2 py-0.5 rounded-md bg-amber-500/10 border border-amber-500/20">
                  <span className="text-[10px] font-bold text-amber-500">{selectedRaids.length} Selected</span>
                </div>
              </div>
            </div>
            <p className="text-[11px] text-zinc-500 font-medium">Choose the raids you want us to complete for you.</p>
          </div>
            ...
            {OSRS_RAIDS.map((raid) => (
              <button
                ...
                className={cn(
                  ...
                  selectedRaids.includes(raid.id)
              ...
              >
                ...
                <span className={cn(
                  ...
                  selectedRaids.includes(raid.id) ? "text-zinc-950" : "text-zinc-100"
                )}>{raid.label}</span>
                <div className={cn(
                  ...
                  {selectedRaids.includes(raid.id) && <Check className="w-4 h-4 text-amber-500 stroke-[3]" />}
                </div>
              </button>
            ))}
          </div>
