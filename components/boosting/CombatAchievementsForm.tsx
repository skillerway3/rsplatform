...
import { OSRS_COMBAT_ACHIEVEMENTS } from '@/data/boosting/osrs-combat-achievements';
import { AccountTypeSelector } from './AccountTypeSelector';

interface CombatAchievementsFormProps {
  onUpdate: (summary: { service: string; details: string[] }) => void;
}

export function CombatAchievementsForm({ onUpdate }: CombatAchievementsFormProps) {
  ...
  useEffect(() => {
    const selectedTierLabels = OSRS_COMBAT_ACHIEVEMENTS
      .filter(t => selectedTiers.includes(t.id))
      .map(t => t.label);

    const details = [
      `Tiers: ${selectedTierLabels.length > 0 ? selectedTierLabels.slice(0, 2).join(', ') + (selectedTierLabels.length > 2 ? ` (+${selectedTierLabels.length - 2})` : '') : 'None'}`,
  ...
  }, [selectedTiers, accountType, additionalInfo, files, onUpdate]);
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div className="space-y-1">
              <label className="text-[10px] font-black text-zinc-600 uppercase tracking-[0.3em]">Select Tiers</label>
              <p className="text-[11px] text-zinc-500 font-medium">Choose the achievement levels to unlock</p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {OSRS_COMBAT_ACHIEVEMENTS.map((tier) => (
              <button
                key={tier.id}
              ...
              >
                ...
                <span className={cn(
                  ...
                  selectedTiers.includes(tier.id) ? "text-zinc-950" : "text-zinc-100"
                )}>{tier.label}</span>
                <div className={cn(
                  ...
                  {selectedTiers.includes(tier.id) && <Check className="w-4 h-4 text-amber-500 stroke-[3]" />}
                </div>
              </button>
            ))}
          </div>
