...
export function BoostingOrderCard({
  options,
  onOptionChange,
  summary,
  className,
}: BoostingOrderCardProps) {
  const { user } = useAuth();
  ...
  const handleSubmit = async () => {
    ...
    if (!summary.service?.trim() || !summary.details?.length) {
      setSubmitError('Please select at least one service option before submitting.');
      return;
    }
    ...
    try {
      ...
      const payload = {
        buyer_id: user.id,
        category: (summary.category || 'services').toLowerCase(),
        game: summary.game || 'OSRS',
        title: `Request for ${summary.service}`,
        description: [
          `Service: ${summary.service}`,
          `Details: ${summary.details.join(', ')}`,
          `Options: ${selectedOptions.length > 0 ? selectedOptions.join(', ') : 'None'}`,
        ].join('\n'),
        status: 'open',
        expires_at: expiresAt.toISOString(),
      };

      console.log('Submitting boosting request...', payload);
...
const result: any = await withTimeout(requestPromise, 15000);
const { data, error } = result || {};
      ...
      const targetUrl = `/orders/${data.id}`;
      console.log('Request submitted successfully, redirecting to:', targetUrl);

      // Use a hard navigation here so we do not stay trapped in a client-side UI state.
  ...
  return (
    <div
      className={cn(
        'sticky top-32 space-y-6 animate-in fade-in slide-in-from-right-8 duration-1000 group/sticky',
        className
      )}
    >
      ...
      <div className="bg-zinc-900/40 border border-zinc-800/50 rounded-[2.5rem] p-8 backdrop-blur-xl shadow-[0_32px_64px_-16px_rgba(0,0,0,0.5)] relative overflow-hidden group">
        ...
        <div className="flex items-center justify-between mb-10">
          <div className="flex flex-col gap-1">
            ...
            <span className="text-[9px] text-zinc-600 font-bold uppercase tracking-widest ml-0.5">
              Marketplace Request
            </span>
          </div>
        ...
        <div className="space-y-5 mb-12">
          {toggleOptions.map((opt) => (
            <div
              key={opt.key}
              className="flex items-center justify-between group/opt p-2 -mx-2 rounded-xl hover:bg-white/[0.02] transition-colors"
            >
              <div className="flex flex-col gap-0.5">
                <span className="text-[10px] font-black text-zinc-400 group-hover/opt:text-zinc-200 transition-colors uppercase tracking-widest">
                  {opt.label}
                </span>
              ...
              <button
                ...
                className={cn(
                  'relative w-11 h-6 rounded-full transition-all duration-500 p-1 border disabled:opacity-60 disabled:cursor-not-allowed',
        ...
        <div className="relative mb-12">
          ...
          <div className="relative p-4 space-y-8">
            <div className="flex items-center justify-between border-b border-zinc-800/50 pb-4">
              <h4 className="text-[9px] font-black text-zinc-600 uppercase tracking-[0.5em]">
                Selected Services
              </h4>
            ...
            <div className="space-y-6">
              <div className="flex items-start gap-4">
                ...
                <div className="flex flex-col gap-1.5">
                  <span className="text-[12px] font-black text-zinc-100 uppercase tracking-[0.15em] leading-none">
                    {summary.service || 'No service selected'}
                  </span>
              ...
              {summary.details.length > 0 ? (
                <div className="space-y-4 pl-10 border-l-2 border-zinc-800/50 ml-4">
                  {summary.details.map((detail, idx) => (
                    <div key={`${detail}-${idx}`} className="flex items-start gap-3 group/detail relative">
                      ...
                      <span className="text-[11px] font-bold text-zinc-500 leading-relaxed uppercase tracking-wide group-hover/detail:text-zinc-300 transition-colors">
                        {detail}
                      </span>
                    </div>
                  ))}
                ...
                <div className="pl-10 ml-4 py-6 border-l-2 border-dashed border-zinc-800/50">
                  <p className="text-[10px] text-zinc-600 font-medium italic uppercase tracking-widest">
                    Awaiting selection...
                  </p>
                </div>
              )}
        ...
        <div className="space-y-6">
          <Button
            ...
            className={cn(
              ...
              summary.details.length > 0 && !isSubmitting
      ...
      <div className="bg-emerald-500/[0.03] border border-emerald-500/10 rounded-[2rem] p-6 flex items-start gap-5 backdrop-blur-md relative overflow-hidden group/shield shadow-xl">
        ...
        <div className="relative z-10">
          <p className="text-[11px] font-black text-emerald-500 mb-1.5 uppercase tracking-[0.3em] text-left">
            Buyer Protection
          </p>
          <p className="text-[10px] text-zinc-500 leading-relaxed text-left font-medium tracking-tight">
            Your payment is held in secure escrow and only released upon successful
            service completion. 100% satisfaction guaranteed or full refund.
          </p>
        </div>
      </div>
    </div>
