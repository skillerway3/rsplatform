...
function getLucideIcon(iconName?: string, fallback: LucideIcon = LucideIcons.Gamepad2): LucideIcon {
    ...
    <div className="space-y-10">
      <div>
        <h2 className="text-2xl font-light mb-6 flex items-center gap-3">
          <span className="w-8 h-8 rounded-full bg-amber-500/10 text-amber-500 flex items-center justify-center text-sm font-bold border border-amber-500/20">
            1
          </span>
          Select Game
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {GAMES.map((game) => {
            const Icon = getLucideIcon(game.icon, LucideIcons.Gamepad2);
      ...
      <div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {CATEGORIES.map((category) => {
            const Icon = getLucideIcon(category.icon, LucideIcons.Package);
