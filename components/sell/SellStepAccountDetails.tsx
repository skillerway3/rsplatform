...
export function SellStepAccountDetails({ formData, updateFormData }: SellStepAccountDetailsProps) {
  const accountMetadata = formData.accountMetadata || {
    build: '',
    type: '',
    loginMethod: '',
    totalLevel: '',
  ...
  const updateMetadata = (data: Partial<typeof accountMetadata>) => {
    updateFormData({
      accountMetadata: { ...accountMetadata, ...data }
    });
  };

  const toggleTag = (tag: string) => {
    const newTags = accountMetadata.tags.includes(tag)
      ? accountMetadata.tags.filter((t: string) => t !== tag)
      : [...accountMetadata.tags, tag];
    ...
    <div className="space-y-12 max-w-4xl mx-auto">
      ...
      <section className="space-y-6">
        <div className="flex items-center gap-3 pb-2 border-b border-zinc-800/50">
          ...
          <div>
            <h2 className="text-xl font-medium text-zinc-100">Account Specifications</h2>
            <p className="text-sm text-zinc-500">Define the core attributes of your account</p>
          </div>
        </div>
          ...
          <div className="space-y-2.5">
            ...
            <div className="relative group">
              <select 
                ...
                onChange={(e) => updateMetadata({ loginMethod: e.target.value })}
                className="w-full bg-zinc-900/40 border border-zinc-800/80 rounded-xl h-12 px-4 focus:outline-none focus:border-amber-500/50 focus:bg-zinc-900/60 transition-all text-zinc-200 appearance-none cursor-pointer"
              >
          ...
          <div className="space-y-2.5">
            ...
            <div className="relative group">
              <select 
                ...
                className="w-full bg-zinc-900/40 border border-zinc-800/80 rounded-xl h-12 px-4 focus:outline-none focus:border-amber-500/50 focus:bg-zinc-900/60 transition-all text-zinc-200 appearance-none cursor-pointer"
              >
                <option value="" disabled>Select Total Level</option>
                {TOTAL_LEVELS.map(l => <option key={l} value={l}>{l}</option>)}
              </select>
              <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-600 pointer-events-none group-focus-within:text-amber-500 transition-colors" />
            </div>
          </div>
      ...
      <section className="space-y-6">
        <div className="flex items-center gap-3 pb-2 border-b border-zinc-800/50">
          ...
          <div>
            ...
            <p className="text-sm text-zinc-500">Quickly highlight key features for buyers</p>
          </div>
        </div>
          ...
          {TAGS.map(tag => (
            <button
              ...
              className={cn(
                ...
                accountMetadata.tags.includes(tag)
            ...
            >
              <div className={cn(
                ...
                {accountMetadata.tags.includes(tag) && <Check className="w-3 h-3 text-zinc-900 stroke-[3]" />}
              </div>
      ...
      <section className="space-y-6">
        <div className="flex items-center gap-3 pb-2 border-b border-zinc-800/50">
          ...
          <div>
            <h2 className="text-xl font-medium text-zinc-100">Account Highlights</h2>
            <p className="text-sm text-zinc-500">List the most valuable features of your account</p>
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="space-y-2.5">
            ...
            <textarea
              ...
              onChange={(e) => updateMetadata({ highlights: e.target.value.split('\n') })}
            ...
            />
            <p className="text-[10px] text-zinc-600 italic">Tip: Be specific. High-value items and achievements sell accounts faster.</p>
          </div>
