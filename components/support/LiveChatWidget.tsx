...
const BUYER_FAQS: Record<string, FAQItem[]> = {
  order_process: [
    {
      question: 'Question about order',
      answer:
        'For questions about an active order, please visit your Order Details page and use the "Message Seller" button for direct communication. If you have a dispute, you can use the "Open Dispute" button after delivery.',
    },
  ],
  verification: [
    {
      question: 'Questions about verification process',
      answer:
        'Verification is required for all sellers to ensure marketplace safety. You can track your verification status in your Seller Dashboard. Most reviews are completed within 24-48 hours.',
    },
  ],
  security: [
    {
      question: 'Account and security questions',
      answer:
        'To keep your account secure, never share your password or click on suspicious links. You can update your security settings and enable 2FA in your profile settings.',
    },
  ],
  violations: [
    {
      question: 'I want to report a violation',
      answer:
        'We take marketplace integrity seriously. Please provide the Order ID or Username of the person you are reporting, along with evidence, and our safety team will investigate.',
    },
  ],
  wallet: [
    {
      question: 'Withdraw money from my wallet',
      answer:
        'Note that all withdrawals are made to your original payment method. Due to payment provider processing time it may take up to 3 business days for money to reach you.',
    },
  ],
};
...
const SELLER_FAQS: Record<string, FAQItem[]> = {
  disputes: [
    {
      question: 'Questions about offers and disputes',
      answer:
        'If a buyer opens a dispute, provide all evidence of delivery in the order chat. Our moderators will review the evidence and make a final decision within 72 hours. For offer questions, ensure your terms are clear in the message.',
    },
  ],
  verification: [
    {
      question: 'Questions about seller verification',
      answer:
        'To become a verified seller, you must provide valid government ID and a selfie. Once submitted, our team will review your application within 48 hours. This is mandatory for all sellers.',
    },
  ],
  feedback: [
    {
      question: 'I want to report abusive feedback',
      answer:
        'If you believe a review is abusive or violates our terms, please report it to our support team with the Order ID. We will investigate and remove it if it meets our removal criteria.',
    },
  ],
  violations: [
    {
      question: 'I want to report a violation',
      answer:
        'Marketplace integrity is our priority. Report any suspicious behavior, off-platform payment requests, or harassment immediately with evidence.',
    },
  ],
  delete_account: [
    {
      ...
      answer:
        'To delete your account, ensure all active orders are completed and your balance is zero. Contact support to initiate the permanent deletion process.',
    },
  ],
};
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on(
        ...
        async (payload) => {
          if (!payload.new || !('id' in payload.new) || !payload.new.id) return;
          ...
          if (newMessage) {
            ...
            const messageWithSender: Message = {
              ...
              sender_type:
                newMessage.sender_type === 'admin' || newMessage.sender_type === 'system'
              ...
              sender: senderProfile
                ? {
                    username: String(senderProfile.username ?? ''),
                    avatar_url:
                      senderProfile.avatar_url != null ? String(senderProfile.avatar_url) : null,
                  }
                : undefined,
            };

            setMessages((prev) => {
              if (prev.find((m) => m.id === messageWithSender.id)) return prev;
  ...
  const handleLiveAgentRequest = async (topic?: string) => {
    ...
    try {
      ...
      if (user) {
        ...
        currentThreadId = existingThread?.id ?? null;
      } else {
        const { data: guestThreadId, error: rpcError } = await supabase.rpc(
          'get_guest_open_thread',
          {
        ...
        currentThreadId = guestThreadId ?? null;
      }
      ...
      if (user) {
        ...
        if (msgs && msgs.length > 0) {
          ...
          const profileMap = (profiles ?? []).reduce<Record<string, MessageSender>>(
            (acc, p: ProfileRow) => {
              acc[p.id] = {
                username: p.username,
          ...
          const msgsWithSender: Message[] = msgs.map((m) => ({
            id: String(m.id),
            ...
            sender: profileMap[String(m.sender_id)],
          }));
        ...
        const transformedMsgs: Message[] = ((msgs ?? []) as GuestMessageRow[]).map((m) => ({
          id: String(m.id),
          sender_id: String(m.sender_id),
          content: String(m.content ?? ''),
          sender_type:
          ...
          sender: m.sender_username
            ? {
                username: String(m.sender_username),
                avatar_url:
                  m.sender_avatar_url != null ? String(m.sender_avatar_url) : null,
              }
            : undefined,
        }));
                  ...
                  >
                    ...
                    {(step === 'buyer'
                      ? [
                          ...
                          {
                            id: 'verification',
                            label: 'Questions about verification process',
                            icon: ShieldCheck,
                          },
                          {
                            id: 'security',
                            label: 'Account and security questions',
                            icon: AlertCircle,
                          },
                          {
                            id: 'wallet',
                            label: 'Withdraw money from my wallet',
                            icon: HelpCircle,
                          },
                          {
                            id: 'violations',
                            label: 'I want to report a violation',
                            icon: AlertCircle,
                          },
                          {
                            id: 'live_agent',
                      ...
                      : [
                          {
                            id: 'disputes',
                            label: 'Questions about offers and disputes',
                            icon: AlertCircle,
                          },
                          {
                            id: 'verification',
                            label: 'Questions about seller verification',
                            icon: ShieldCheck,
                          },
                          {
                            id: 'feedback',
                            label: 'I want to report abusive feedback',
                            icon: HelpCircle,
                          },
                          {
                            id: 'violations',
                ...
                {step === 'faq' && category && (
                  ...
                  >
                    ...
                    <div className="pt-6 border-t border-white/5">
                      <p className="text-zinc-500 text-[9px] font-black uppercase tracking-widest text-center mb-4">
                        Still need help?
                      </p>
                      <Button
                        variant="outline"
                ...
                {step === 'chat' && (
                  ...
                  >
                      ...
                      {messages.map((msg) => (
                        ...
                        >
                          ...
                          <span className="text-[8px] font-black text-zinc-600 uppercase tracking-widest px-1">
                            {msg.sender_type === 'admin'
                              ? 'Support Agent'
                              : msg.sender?.username || (user ? 'You' : 'Guest')}{' '}
                            •{' '}
                    ...
                    <form
                      ...
                      className="p-4 bg-zinc-950/50 border-t border-white/5 flex items-center space-x-3"
                    >
                      <input
                        type="text"
