...
export function ListingInfo({ listing }: ListingInfoProps) {
  ...
  const deliveryMethod = safeListing.deliveryMethod || 'Manual';
  const deliveryTime = safeListing.deliveryTime || 'Fast';
  const stock = safeListing.stock ?? 0;
  const description = safeListing.description || 'No description provided.';
  const title = safeListing.title || 'Untitled Listing';
  ...
  return (
    <div className="space-y-12">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-10">
          ...
          <div className="flex space-x-4 shrink-0">
            <Button
              variant="secondary"
              size="icon"
              className="rounded-[1.5rem] w-16 h-16 bg-zinc-900/50 border-zinc-800 hover:border-amber-500/50 transition-all group shadow-xl"
            >
        ...
        <div className="flex flex-wrap gap-12 py-12 border-y border-zinc-900/50">
          <div className="flex items-center space-x-6">
            ...
            <div>
              ...
              <div className="text-base text-zinc-100 font-black uppercase tracking-tight">
                {deliveryMethod}
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-16">
        <div className="space-y-8">
          <h2 className="text-[11px] font-black text-zinc-600 uppercase tracking-[0.3em]">
            Description
          </h2>
