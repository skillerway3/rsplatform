...
export function ChatModal({
  ...
  const [otherUser, setOtherUser] = useState<OtherUserRow | null>(null);
    ...
    const setupChat = async () => {
      ...
      try {
        ...
        if (isMounted && profile) {
          setOtherUser(profile as OtherUserRow);
        }
        ...
        let thread: ThreadRow | null = (existingThread as ThreadRow | null) ?? null;
        ...
        if (!thread) {
          ...
          thread = (newThread as ThreadRow | null) ?? null;
        }
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    setSending(true);
    try {
      const { error } = await supabase.from('buyer_request_messages').insert({
        thread_id: threadId,
        sender_id: user.id,
          ...
          >
            <div className="p-6 border-b border-zinc-800 flex items-center justify-between bg-zinc-900">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h3 className="text-sm font-black text-white uppercase tracking-widest truncate max-w-[200px]">
                    {otherUser?.username || title}
                  </h3>
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    {otherUser?.is_trusted_seller && (
                      <div className="flex items-center gap-1 px-1 py-0.5 bg-amber-500/10 border border-amber-500/20 rounded-md">
                        ...
                        <span className="text-[6px] font-black text-amber-500 uppercase tracking-widest">
                          Trusted
                        </span>
                      </div>
                    )}
            ...
            <form onSubmit={handleSendMessage} className="p-6 border-t border-zinc-800 bg-zinc-900">
